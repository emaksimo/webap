...
ncol=compteur-1;
 ...
 plot(direction(nbline,:)',zdirection(nbline,:)','sqk');
 hold on;
...
%[Ax,zvhh,dirr,zdirr]=plotyy(vh(nbline,:)',zvh(nbline,:)',direction(nbline,:)',zdirection(nbline,:)','plot');
...
% title('WIND DIRECTION AND HORIZONTAL WIND SPEED profil');
 
 tit1=[location,' - ',datestr(datenum(aa(1),mm(1),j(1),hh((nbline)),mn((nbline)),ss(nbline)),13),...
     ' ',temps,', ',datestr(datenum(aa(nbline),mm(nbline),jj(nbline)),20)];
...
clear im_dir_vh im_dir_vz im_dir_dvr im_dir_cnr max_z1 %direction  zdirection
